<section x-data="orderDetailPage()" class="mx-auto max-w-4xl animate-rise">
    <div class="rounded-[2rem] border border-bronze/15 bg-white/80 p-6 shadow-glow sm:p-8">
        <div class="text-sm uppercase tracking-[0.35em] text-bronze/70">订单详情</div>
        <h1 class="mt-3 font-display text-4xl text-ink">查看订单信息</h1>

        <template x-if="loading">
            <div class="mt-6 rounded-[1.5rem] bg-parchment/65 p-5 text-sm text-ink/60">正在读取订单详情，请稍候。</div>
        </template>

        <template x-if="!loading && errorMessage">
            <div class="mt-6 rounded-[1.5rem] border border-rose/20 bg-rose/5 p-5 text-sm text-rose" x-text="errorMessage"></div>
        </template>
            ...
            <div class="mt-6 space-y-6">
                <div class="rounded-[1.5rem] bg-parchment/65 p-5">
                    <div class="grid gap-3 text-sm text-ink/70 sm:grid-cols-2">
                        ...
                        <div class="sm:col-span-2">应付金额：<span class="font-medium text-bronze">¥<span x-text="formatMoney(order.payable_amount || 0)"></span></span></div>
                    </div>
                </div>
                ...
                <template x-if="orderHasShippingInfo(order)">
                    <div class="rounded-[1.5rem] border border-teal/15 bg-teal/5 p-5">
                        ...
                        <div class="mt-4 grid gap-3 text-sm text-ink/70 sm:grid-cols-2">
                            ...
                            <div class="sm:col-span-2">物流单号：<span class="font-medium text-ink" x-text="order.shipping_no || '未填写'"></span></div>
                        </div>
                    </div>
                </template>
        ...
        <div class="mt-6 flex gap-3">
            <?php if (!empty($currentUser)): ?>
                <a href="/mall/profile?tab=orders" class="rounded-full bg-bronze px-5 py-3 text-sm text-white shadow-card">返回我的订单</a>
            <?php endif; ?>
